    ...
    async execute(interaction: CommandInteraction): Promise<void> {
        ...
        switch (subcommand){
            case "set":
                ...
                let callAll = interaction.options.getBoolean("callall") || false;
                ...
                await interaction.reply("Okay, I'll remind you soon~\nIn the event that you wish to no longer be reminded of this timer, use /reminder delete " + this.m_id.toString());
                this.timeList[this.m_id] = true;
                this.m_id += 1;
                break;
            case "delete":
                //variable
                let c_id = interaction.options.getInteger("id") || this.m_id - 1;
                //clear the reminder
...
const setSecond = new SlashCommandIntegerOption()
    .setName("second")
    .setDescription(
        "Set for how many seconds (required field)"
    )
    .setRequired(true)
...
const callAll = new SlashCommandBooleanOption()
    .setName("callall")
    .setDescription(
        "whether or not to remind everyone"
    )
    .setRequired(false)
