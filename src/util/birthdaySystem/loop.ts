...
export async function BirthdayMessage(client: Client) {
  ...
  schedule("0 * * * *", async () => {
    ...
    const possibleUsers = date.getUTCHours() == 0 ?
      getMembersWithNoTimeZone(defaultGuild) || [] :
      getMembersWithTimeZone(`UTC${(timezone > 0 ? "+" + timezone : (timezone < 0 ? timezone : ""))}`, defaultGuild) || []
    ...
    for (const user of possibleUsers) {
      ...
      if (
        ...
        await defaultGuild.systemChannel?.send({ content: user.toString(), embeds: [embed] }).catch((e) => null);
      }
    }
  });
}
