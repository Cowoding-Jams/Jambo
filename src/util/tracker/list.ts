...
export async function createList(filter:string, offset:number) {
    ...
    keys.forEach((e) => {
        ...
        switch (filter) {
            case "0":
            case "1":
                if (!final.has(game)) {final.set(game, playtime)}
                else {
            ...
            case "2":
            case "3":
                if (!final.has(game)) {final.set(game, logs)}
                else {
                    let time = final.get(game)
                    if (time) time += logs
                    else return
                ...
                return
            case "4":
                if (!final.has(game)) {final.set(game, lastplayed)}
                else {
                    let time = final.get(game)
                    if (!time) return
                    if (lastplayed > time) {final.set(game, lastplayed)} 
                }
                return
            case "5":
                if (!final.has(game)) {final.set(game, firstplayed)}
                else {
                    let time = final.get(game)
                    if (!time) return
                    if (lastplayed < time) {final.set(game, firstplayed)}
                }
                return
        }
    })
