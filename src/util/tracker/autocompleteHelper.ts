...
export function user(option:string, userid:string | number | boolean | undefined, game:string | number | boolean | undefined) {
    ...
    if (option == "game") {
        if (typeof userid == "string") {
            ...
            results.push(...(typeof game == "string" ? ["playtime", "logs"] : ["general statistics", "top 5 most played games", "top 5 most logged games", "latest 5 logs"]));
    }
...
export function blacklist(action:string) {
    ...
    if (action == "add") {
        ...
        results.push(
            ...trackerGames.keyArray().filter((e) => (action=="add" ? !trackerBlacklist.get("")?.find((e2) => e2 == e) : !!trackerBlacklist.get("")?.find((e2) => e2 == e)))
        )
    } else {
