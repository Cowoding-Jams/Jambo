...
import { durationToReadable, discordTimestamp } from "../misc/time"
import { Duration } from "luxon";
import { trackerUsers, trackerLogs, TrackerSublog, trackerGames, trackerBlacklist } from "../../db"
...
function makeEmbed(
        target:GuildMember, requester:User, 
        mostLogged:string, mostPlayed:string, 
        latestLogs:string, mostPlaytime:string, 
        firstSeen:number, totalLogs:number, 
        totalPlaytime:string, playtimePer:string) {
    return new EmbedBuilder()
        .setAuthor({name:"Jambo", url:"https://github.com/Cowoding-Jams/Jambo", iconURL:"https://github.com/Cowoding-Jams/Jambo/blob/main/images/Robot.png"})
        ...
        .addFields(
            ...
            {inline:false, name:"_ _",                        value:"_ _"},
            {inline:true, name:"Record range",                value:`${discordTimestamp(Math.floor(firstSeen/1000))} -> ${discordTimestamp(Math.floor(Date.now()/1000))}(now)\n${makeTimeString(Date.now() - firstSeen)}`},
            {inline:false, name:"_ _",                        value:"_ _"},
...
export async function userStats(interaction:ChatInputCommandInteraction) {
    ...
    if (member == undefined) {await interaction.reply({content:"User not found", ephemeral:true}); return}
    const games = sortGamesPlaytime(member.id)
    const logs = sortGamesLogs(member.id)
    if (!logs || !games) {await interaction.reply({content:"User not enogth logs", ephemeral:true}); return}
    ...
    if (!userDB) {await interaction.reply({content:"User not found", ephemeral:true}); return}
    const firstSeen = trackerLogs.get(userDB.firstlog)?.time
    if (!firstSeen) {await interaction.reply({content:"User not found", ephemeral:true}); return}
    const range = Date.now() - new Date(firstSeen).getTime()
    const playtimePerLog = getPlaytimePerLog(member.id)
    if (playtimePerLog == 0) {await interaction.reply({content:"User not found", ephemeral:true}); return}
    const latest = playtimePerLog.at(-1)
    if (!latest) {await interaction.reply({content:"User not found", ephemeral:true}); return}

    const embed = makeEmbed(
        member, 
        interaction.user, 
        logs.slice(0, 5).map(game => `${game.id}: ${game.logs}`).join("\n"),
...
export async function userLast(interaction:ChatInputCommandInteraction) {
    ...
    if (!dbEntry) {await interaction.reply({content:"User not enogth logs", ephemeral:true}); return}
    ...
    logs.forEach(log => {
        ...
        if (!entry) return
        fields.push({inline:true, name:entry.gameName, value:`time: <t:${Math.floor(new Date(entry.time).getTime()/1000)}:d><t:${Math.floor(new Date(entry.time).getTime()/1000)}:t>\nplayed time: ${makeTimeString(entry.playtime)}`})
    })
...
export async function playtime(interaction:ChatInputCommandInteraction) {
    ...
    const db = trackerUsers.get(member.id)?.games.filter(game => game.id.toLocaleLowerCase() == targetGame.toLocaleLowerCase())

    if (!db) {await interaction.reply({content:"User not existing or not played game", ephemeral:true}); return}
...
export async function latest(interaction:ChatInputCommandInteraction) {
    ...
    for (let i = trackerLogs.count; i > trackerLogs.count - config.activityLogRange; i--) {
        ...
        fields.push({
            inline: true,
            name: data.gameName,
            value: `user: <@${data.userid}>\ntime: <t:${Math.floor(new Date(data.time).getTime()/1000)}:d><t:${Math.floor(new Date(data.time).getTime()/1000)}:t>\ntime played: ${makeTimeString(data.playtime)}`
        })
    }
...
export async function stats(interaction:ChatInputCommandInteraction) {
    ...
    const mostLoggedGame = trackerGames.array().sort((a, b) => a.logs - b.logs).reverse().slice(0, 5).map(game => `${trackerLogs.get(game.lastlogs[0])?.gameName}: ${game.logs}`).join("\n")
    ...
    const latestLogs = trackerLogs.array().reverse().slice(0, 5).map(log => `<t:${Math.floor(new Date(log.time).getTime()/1000)}:d><t:${Math.floor(new Date(log.time).getTime()/1000)}:t> <@${log.userid}> ${log.gameName}: ${makeTimeString(log.playtime)}`).join("\n")
    const totalPlaytime = trackerGames.array().map(game => game.playtime).reduce((partialSum, a) => partialSum + a, 0)
    const totalLogs = trackerGames.array().map(game => game.logs).reduce((partialSum, a) => partialSum + a, 0)
    const firstSeen = new Date(trackerLogs.array()[0].time).getTime()
    const range = Date.now() - firstSeen
    const games = trackerGames.count
    const playtimePer = `day: ${makeTimeString(Math.round(totalPlaytime/(range/(86400 * 1000))))}\nweek: ${makeTimeString(Math.round(totalPlaytime/(range/604800000)))}\nmonth: ${makeTimeString(Math.round(totalPlaytime/(range/2628000000)))}\ngame: ${makeTimeString(Math.round(totalPlaytime/games))}\nlog: ${makeTimeString(Math.round(totalPlaytime/totalLogs))}`
    const tmp = trackerGames.array().sort((a, b) => b.playtime/b.logs - a.playtime/b.logs)[0]
    const mostPlaytime = `${trackerLogs.get(tmp.lastlogs[0])?.gameName}: ${makeTimeString(tmp.playtime/tmp.logs)} - ${tmp.logs} logs\nTotal playtime: ${makeTimeString(tmp.playtime)}`

    const embed = new EmbedBuilder()
        .setTitle("System stats")
        .setColor(0)
        .setFooter({text:`Requested by ${interaction.user.tag}`, iconURL: interaction.user.displayAvatarURL({size:16})})
        .setTimestamp(Date.now())
        .addFields(
...
export async function gameStats(interaction:ChatInputCommandInteraction) {
    ...
    if (!db) {await interaction.reply({content: "not existingngngng", ephemeral:true}); return}

    const mostPlayed = db.users.sort((a, b)=> b.playtime - a.playtime).slice(0, 5).map(user => `<@${user.id}>: ${makeTimeString(user.playtime)}`).join("\n")
    ...
    const mostPlaytime = `<@${tmp.id}>: ${makeTimeString(tmp.playtime/tmp.logs)} - ${tmp.logs} logs\nTotal playtime: ${makeTimeString(tmp.playtime)}`
...
export async function gameLast(interaction:ChatInputCommandInteraction) {
    ...
    if (!db) {await interaction.reply({content:"Game not found or no logs", ephemeral:true}); return}
    ...
    await logs.forEach(async log => {
        ...
        if (!user) return
        fields.push({inline:true, name:user.username, value:`time: <t:${Math.floor(new Date(entry.time).getTime()/1000)}:d><t:${Math.floor(new Date(entry.time).getTime()/1000)}:t>\nplayed time: ${makeTimeString(entry.playtime)}`})
    })
...
export async function gameTop(interaction:ChatInputCommandInteraction) {
    const target = interaction.options.getString('game', true)
    if (!target) {await interaction.reply({content:"Game not found", ephemeral:true}); return}
    const db = trackerGames.get(target)
    if (!db) {await interaction.reply({content:"Game not found or no log", ephemeral:true}); return}
    ...
    const users = db.users.sort((a, b) => filter == "playtime" ? (b.playtime - a.playtime) : (b.logs - a.logs)).splice(0, 5)
    let fields:APIEmbedField[] = []
...
export async function remBlacklist(interaction:ChatInputCommandInteraction) {
    ...
    if (db?.find(g => g.toLowerCase() == game.toLowerCase())) {
        trackerBlacklist.set("", db.filter(g=> g.toLowerCase() != game.toLowerCase()))
